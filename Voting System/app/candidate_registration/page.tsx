...
export default function CandidateRegistration() {
  ...
  const handleNIDUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.valid && data.age >= 18) {
        setNidVerified(true);
        setVerifiedAge(data.age);
        setErrors((prev) => ({ ...prev, nid: "" }));
      } else {
        setNidVerified(false);
        setErrors((prev) => ({
          ...prev,
          nid: "Candidate must be at least 18 years old",
        }));
      }
    } catch (error) {
      console.error(error);
  ...
  const validateForm = () => {
    ...
    if (!candidateForm.age || parseInt(candidateForm.age) < 18) {
    ...
    if (!nidVerified) {
      newErrors.nid = "NID verification required";
    }
        ...
        <div className="text-center space-y-8 max-w-md mx-auto px-4">
          <div className="space-y-4">
            ...
            <p className="text-xl text-muted-foreground">
              Register new candidates for voting
            </p>
          </div>
          <button
        ...
        <Card className="max-w-md mx-auto">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Only the organizer can register new candidates.
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background/50 to-background">
      <div className="container mx-auto px-4 py-8 pt-24">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-muted-foreground">
            Register new candidates for the voting process
          </p>
        </div>
            ...
            <CardContent>
              ...
              {nidUploading && (
                <p className="text-sm text-muted-foreground">
                  Verifying NID, please wait...
                </p>
              )}
              {errors.nid && (
                <p className="text-sm text-destructive">{errors.nid}</p>
              )}
              {nidVerified && verifiedAge !== null && (
                <p className="text-sm text-emerald-600 font-semibold">
                  ✅ Verified Age: {verifiedAge}
                </p>
              )}
            </CardContent>
          </Card>
        ...
        {nidVerified && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              {fileUrl && candidateForm.name ? (
                <div className="space-y-4">
                  <h2 className="text-2xl font-semibold">Preview</h2>
                  <CandidateCard
                    candidate={{
                      id: candidateArray.length + 1,
                      name: candidateForm.name,
                      image: fileUrl,
                      age: parseInt(candidateForm.age) || 0,
                ...
                <Card className="bg-gradient-to-br from-primary/5 to-accent/5 border-primary/20">
                  ...
                  <CardContent className="space-y-4">
                    <p className="text-muted-foreground">
                      Fill out the form to register a new candidate. A preview
                      will appear here once you provide the required information.
                    </p>
              ...
              {candidateArray.length > 0 && (
                <div className="space-y-4">
                  <h2 className="text-2xl font-semibold">Current Candidates</h2>
                  <div className="grid gap-4 max-h-96 overflow-y-auto">
                    {candidateArray.slice(0, 3).map((candidate) => (
                      <div
                        key={candidate.id}
                    ...
                    {candidateArray.length > 3 && (
                      <div className="text-center text-muted-foreground">
                        +{candidateArray.length - 3} more candidates
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <Alert>
                ...
                <AlertDescription>
                  Only the organizer can register new candidates. All candidate
                  information will be stored on the blockchain and cannot be
